...
(function () {
    ...
    function init() {
        ...
        data_v = document.querySelector('.follow-ctnr .left-part').getAttributeNames()[0]
        //注入
    ...
    const htmls = {
        LIVE__MENU_INJECT: '<div id="plugins_setting" '+data_v+'="" style="margin-right:5px;" role="button" aria-label="插件设置" title="插件设置" class="left-part live-skin-highlight-bg live-skin-button-text dp-i-block pointer p-relative"><!----><!----><span '+data_v+'="" class="follow-text v-middle d-inline-block">插件设置</span><!----><!----></div>'
    };
    //菜单注入
            ...
            Swal.fire({
                ...
                if(result.isDenied){
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            bili_video_id = document.querySelector('.live-player-mounter').lastChild.id
        ...
        for (const mutation of mutationsList) {
          if (mutation.type === 'childList') {
            [...mutation.addedNodes].map(item => {
              ...
              if(item.classList?.contains('mode-roll')) {
                  mmsn_log('目标变更', item);
                  show_dm_ban(item)
              }
            })
