...
window.onload = function () { // 重构为加载时再进行各种操作
    ...
    function time_stamp_to_time(timestamp) { // 时间戳解析 2023/6/19 1:57 由timestamptotime 更改为 time_stamp_to_time
    ...
    function bilibili_run() {
        ...
        function init() {
            ...
            live_tools_log.warn(time_stamp_to_time(load_time) + '时加载脚本')
        ...
        window.setTimeout(function bili_live_menu_inject() {
            ...
            document.querySelector(ids.MENU__LIVE_TOALS_ID).addEventListener('click', function () { // 监听点击"数据菜单"
                now_time = time_stamp_to_time(time_stamp_ten(Date.now())) // 获取现在时间
                Swal.fire({
                    title: '<font size=5>' + time_stamp_to_time(load_time) + '到<br>' + now_time + '的统计</font>',
                    html:
                    ...
                    if (result.isDenied) {
                        var total_json_data = {
                            ...
                            'up': anchor,
                            'up_uid': anchor_uid,
                            'start_time': time_stamp_to_time(load_time),
        ...
        function wss_get() {
            ...
            $.get('https://api.live.bilibili.com/room/v1/Room/room_init?id=' + room_id, function (rddata) { // 获取真实的房间号，因为有些房间是短号，而ws服务器是根据真实房间号来做广播的
                ...
                ws_content.onopen = function () { // 在ws连接成功后
                    ...
                    live_tools_log.warn(time_stamp_to_time(time_stamp_ten(Date.now())) + '时连接websocket服务器成功') // 注意！必须要在5秒内发送正确的验证包，不然会被服务器断开wss连接
                    var auth_bag = {
        ...
        function dm_timeshow(wrapper) {
            ...
            if (wrapper.getAttribute('data-danmaku') != undefined) { // 区分普通弹幕和礼物、系统提示
                if (wrapper.getAttribute('data-image') != undefined) {
                    setTimeout(function () {
                        var dm_send_time = time_stamp_to_time(wrapper.getAttribute('data-ts')) // 获取弹幕发送时间戳
        ...
        function superchat_event(target) {
            setTimeout(function () { // 这里必须设置定时，如果不设置定时，设置属性的步骤会比获取target先执行 曾经可以直接通过username跳转到其主页！
