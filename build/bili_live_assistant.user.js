...
(function () {
    ...
    function copy_this(copy_content) {
    ...
    const no_watermark = $('.web-player-icon-roomStatus')
    no_watermark.remove()
    ...
    window.setTimeout(function attack_player() {
        ...
        // 复制直播流链接
        var right_click_menu_getstreamlink_id = document.querySelector(ids.RIGHT_MENU__CLICK_GET_STREAM_LINK_ID)
        right_click_menu_getstreamlink_id.addEventListener('click', function () {
            $.get('https://api.live.bilibili.com/room/v1/Room/playUrl?cid=' + room_id + '&platform=h5', function (data) {
                const stream_link = data.data.durl[0].url
                copy_this(stream_link)
        ...
        document.querySelector(ids.RIGHT_MENU__CLICK_GET_STREAM_COVER_ID).addEventListener('click', function () {
            $.get('https://api.live.bilibili.com/room/v1/Room/get_info?id=' + room_id, function (data) {
                const cover_url = data.data.user_cover
                Swal.fire({
                    title: '直播间封面',
        ...
        // 删除进入直播间的提示
        document.querySelector(ids.RIGHT_MENU__CLICK_NO_IN_SHOW).addEventListener('click',function(){
            const in_stream_room = $('.brush-prompt')
            in_stream_room.remove()
            document.getElementsByClassName('_web-player-context-menu_'+rdm_id+'')[0].setAttribute('style', 'opacity : 0;')
        });
        // 直播流300秒(5分钟)切片
    ...
    function observeComments(wrapper) {
        if(wrapper.getAttribute('data-danmaku') != undefined){
            if(wrapper.getAttribute('data-ts') == "0"){
                var send_time_show = '<span id="time_menu" style="color:#00D1F1;"><br>你应该知道自己是在什么时候发的弹幕吧！</span>'
                ...
                insert_here = wrapper.querySelector('.danmaku-content')
                $(send_time_show).insertAfter(insert_here);
            }
            var find_ban_words = GM_getValue('ban_word')
            var ban_words = find_ban_words.replace('/', '')
